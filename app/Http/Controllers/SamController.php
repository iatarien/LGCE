...
class SamController extends Controller
{
    ...
    // public function get_cumul1($portefeuille,$filters="",$op="")
    // {   

    //     if($op ==""){
    //         //$query="SELECT id_op,intitule_ar,numero,SUM(to_pay)as somme_pay, sum(montant) as somme FROM engagements LEFT JOIN payments ON payments.id_eng = engagements.id INNER JOIN operations ON engagements.id_op = operations.id WHERE (engagements.type='juridique' OR engagements.type='FSDRS' OR engagements.type='decision' or engagements.type='mixte') AND engagements.date_visa IS NOT NULL GROUP BY engagements.id_op;";
    //         $query="SELECT id_op,portefeuille,AP_init,AP_act,taux,observations,
    //         intitule_ar,numero,source,real_sujet, SUM(montant) as somme,
    //         operations.id as oper_id FROM engagements 
    //         RIGHT JOIN operations ON engagements.id_op = operations.id AND 
    //         engagements.type='eng' AND ";
    ...
    //             $query = $query." operations.portefeuille = '".$portefeuille."' AND"; 
    //         }
    //         $query= $query." engagements.date_visa IS NOT NULL GROUP BY operations.id ";
    //         echo $query."\n\n";

    //     }else{
    //         $query="SELECT id_op,portefeuille,AP_init,AP_act,taux,observations,intitule_ar,numero,source,real_sujet, 
    //         sum(montant) as somme,  operations.id as oper_id FROM engagements RIGHT JOIN operations ON 
    //         engagements.id_op = operations.id AND 
    //         engagements.type='eng' AND ";
    //         if ($portefeuille != "" && $portefeuille != "all" ){
    //             $query = $query." operations.portefeuille ='".$portefeuille."' AND "; 
    //         }
    //         $query = $query." engagements.date_visa IS NOT NULL 
    //         AND engagements.id_op = ".$op." GROUP BY operation.id";
    //     }
    //     //echo $query;
    //     $cumul_net = DB::select( DB::raw($query));
    //     $cumul_total = array();
