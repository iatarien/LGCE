...
function display(engagements,value){
	...
	var tds = '<tr style="  font-weight : bolder;">'+
		        ...
		        '<td style=" width : 22%;"><div>Numéro et intitulé de l\'operation </div></td>'+
				'<td style="width : 10.5%;"><div>Marché</div></td>'+
				'<td style=" width : 10.5%;" id="total"><div>Entreprise</div></td>'+
		        '<td style=" width : 8%;" id="intitule"><div> Date  </div></td>'+
                '<td style=" width : 10.5%;" id="intitule"><div>Lot</div></td>'+
				'<td style=" width : 13%;" id="total"><div>Montant</div></td>'+
				'<td style="width : 6%;"><div>durée</div></td>'+
		        '<td style="width : 7.5%;"><div>Modifier</div></td>'+
				'<td style="width : 9%; text-align : center;"><div>Supprimer</div></td>'+
		      '</tr>';
	...
	for (var i = 0; i < engagements.length; i++) {
		...
		'<tr style="font-weight : bold'+style+'">'+
		    ...
		    if(user_id == op[i].user_id ){
				tds +='<td>'+
		    	'<span><button class="btn btn-sm btn-primary"  onclick="document.location.href=\'/modifier_deal/'+op[i].deal_id+'\'">Modifier</button></span>'+
		        '</td>';
			}else{
				tds +='<td>'+
		    	'<span><button disabled class="btn btn-sm btn-primary"  onclick="document.location.href=\'/'+type+'/'+op[i].eng_id+'\'">Modifier</button></span>'+
		        '</td>';
			}
			if(op[i].user_id == user_id && !op[i].num_visa || op[i].num_visa == null || op[i].num_visa == ""){
				tds +='<td style="text-align : center;">'+
			'<span><button class="btn btn-sm btn-danger"   onclick="supprimer(\'/delete_deal/'+op[i].deal_id+'\')">Supprimer</button></span>'+
			'</td>';
			}else{
				tds +='<td style="text-align : center;">'+
