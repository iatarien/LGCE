			...
			<div class="  col-sm-3 form-group">
				<input id="e_input" autocomplete="off" dir="rtl" placeholder=" المتعامل المتعاقد" list="ops" class="form-control"  onclick="e_like(this.value)" onkeyup="e_like(this.value)" > 
                    <div id="myDropdown1" class="dropdown-content" style="display: none;">
                      @foreach ($es as $e)
                      <span class="es_clss" style="cursor: pointer; text-align : right;" onclick="filter_e('{{$type}}','{{ $e->id }}1989raouf1989{{ $e->name }}')">{{ $e->name  }}</span>
                      @endforeach
                    </div>
			</div>
...
function display(engagements,value){
	...
	var tds = '<tr style="  font-weight : bolder;">'+
		       ...
		       '<td style="width : 7%; text-align : center;"><div>إختيار</div></td>'+
		      '</tr>';
		...
		'<tr style="font-weight : bold'+style+'">'+
		    ...
		    if(type == "avenant"){
				if(op[i].user_id == user_id || ville =="Medea"){
					...
					tds +='<td>'+
					'<span><button class="btn btn-success" disabled  onclick="document.location.href=\'/ajouter_avenant/'+op[i].deal_id+'\'">إختيار</button></span>'+
					'</td>';
				}
