...
<div id="main" class="main row">
	<div class="col-lg-12">
		<div class="row">
			<div class="col-sm-1 form-group" style="visibility : hidden;">
			...
			<div class="col-sm-1 form-group">
				...
				<img style="max-width : 38%;" src="{{ url('img/print.png') }}">
				</button>
				@endif
			</div>
...
function display(engagements,value){
	...
	var tds = '<tr style="  font-weight : bolder;">'+
		        ...
		        '<td style=" width : 12%;" id="intitule"><div> الموضوع  </div></td>'+
                '<td style=" width : 10%;" id="intitule"><div>المقـــاول</div></td>'+
				'<td style="width : 15%;"><div> القيمة </div></td>'+
	...
	for (var i = 0; i < engagements.length; i++) {
		...
		'<tr style="font-weight : bold '+style+'">'+
			...
			if(op[i].travaux_num != null){
				tds +='<td>'+
		        '<span><h5><strong>'+op[i].travaux_type+" رقم "+op[i].travaux_num;
			}else {
				tds +='<td>'+
		        '<span><h5><strong>'+op[i].travaux_type;
			}

			tds+= '<br>بتاريخ :<br> '+op[i].date_pay+'</strong></h5></span>'+
		    '</td>';
			...
			if(user_service =="Comptabilité"){
					...
					'</td>';
					tds +='<td style="text-align : center;">'+
					'<span><button class="btn btn-danger"   onclick="supprimer(\'/delete_pay/'+op[i].p_id+'\')">خذف</button></span>'+
					'</td>';
					}else{
						tds +='<td style="text-align : center;">'+
					'<span><button class="btn btn-primary" disabled >تعديل</button></span>'+
					'</td>';
					tds +='<td style="text-align : center;">'+
