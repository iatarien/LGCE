...
$data = [
    "دراســـات",
    "البنـــــاء",
    "الأشغــال العمــوميــة",
    "الات و تجهيزات الانتاج",
    "عتــاد النقـــل",
    "التكــويــن",
    "ارادات الخــدمــة الخارجيـة",
    "متنــوعـــة : إشهـــــار",
];
...
<section  style="background-color: white; text-align: center; font-size: 14.5px; margin: 20px; margin-top : 5%;" id="fiche">
	<div id="fiche_top">
		...
		<div dir="rtl" style="float: right; text-align : right; width : 100%;">
            ...
            <h3> محل الدفع : <span dir="ltr">{{$bank->bank_acc}} {{$bank->bank}} {{$bank->bank_agc}} </span><br>
		    <span style="visibility : hidden"> محل الدفع : </span><span>{{$bank->bank_user}} </span>
		   	</h3>
            <h3> هيكــل الالتـزام المقتـرحة : </h3>
		</div>
        <div style="width : 100%">
            <table id="numero" style="width : 15%; float : right;">
                <tr>
		...
		<table id="engagement" contenteditable="true" dir="rtl" style="margin-top : 10px;">
			...
			<?php array_shift($titres); $i = 1;?>
            @foreach($data as $t)

            <tr style="text-align :  center; font-weight : bold" dir="ltr">
                <td>{{$i}}</td>
                <td>{{$t}}</td>
				<td id="rub{{$i}}" style="width : 25%;" onclick="showVal(this.id)"></td>
	...
	<table id="numero" style="width : 100%; float : right; text-align : right;" dir="ltr">
		<tr  style=" background-color: rgb(245,245,245) !important; ">
			<td>  المدفوعات الكلية</td>
			<td> الدفع المقترح</td>
    ...
    <table id="numero" style="width : 45%; float : right; text-align : center;">
		<tr>
			<td colspan="1" style="width: 90px; background-color: rgb(245,245,245) !important; ">       
				الحوالة رقم : &emsp;{{$pay->num_mondat}} &emsp; المؤرخة في : {{$pay->date_mondat}} 
			</td>
		</tr>
		<tr>
			<td style="text-align : right; padding-right : 20px;">مقبولة للصرف بتاريخ : </td>
		</tr>
...
function showVal(id){
	document.getElementById(id).innerHTML = "{{ number_format((float)$pay->to_pay, 2, '.', ' ')}}";
}
function retour(){
