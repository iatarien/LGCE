...
<section style="background-color: white; text-align: center;  margin: 20px;" id="fiche">
	<div id="fiche_top">
		...
		<table>
			<tr>
				<th style="width : 5%;">CW</th>
				<th style="width : 5%;">Chapitre</th>
				<th style="width : 20%;">N° Operation </th>
                <th style="width : 10%;">CP notifié {{$year}} </th>
				<th style="width : 20%;">Consommation du {{$start}} au {{$end}}</th>
				<th style="width : 20%;">Mondats au niveau du tresor mois de <br> {{$month}} {{$year}}</th>
                <th style="width : 20%;">Solde de CP</th>
			</tr>
			@foreach($chaps as $chap)
				<tr>
					<td style="border-top : none;" rowspan="{{$chap->n}}">30</td>
					<td rowspan="{{$chap->n}}">{{$chap->ops[0]->chapitre}}</td>
					<td>{{$chap->ops[0]->numero}}</td>
					<td>{{ number_format((float)$chap->ops[0]->montant_cp, 2, '.', ',')}}</td>
					<td >{{ number_format((float)$chap->ops[0]->pay_ant, 2, '.', ',')}}</td>
				...
				@foreach($chap->ops as $op)
					<tr>
						...
						<td >{{ number_format((float)$op->pay_ant, 2, '.', ',')}}</td>
