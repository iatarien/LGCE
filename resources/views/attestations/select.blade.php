...
function filter_e(type="",value){
	...
	}else if(real_type == "att_commune" || real_type == "att_retard" || real_type == "att_error"){
		var type = "decision,eng";
	}
	else{
		var type = "add_att";
	}
	...
	if(real_type == "att_commune" || real_type == "att_retard" || real_type == "att_error"){
...
function subject(type,deal,deal_num,deal_date,annexe,sujet,real_sujet,e){
	...
	if (sujet.includes('1989raouf1989')) {
		sujets = sujet.split("1989raouf1989");
		sujet = sujets[1];
		txt = sujets[0];
	}else{
		txt = "";
...
function display1(engagements,value){
    var user_id = {{ $user->id }};
    var type= "{{ $type }}";
	var tds = '<tr style="  font-weight : bolder;">'+
		        '<td style="width : 2.5%;" ><div>#</div></td>'+
		        '<td style=" width : 23.5%;"><div>رقم العملية</div></td>'+
		        '<td style=" width : 10%;" id="intitule"><div> الموضوع  </div></td>'+
                '<td style=" width : 10%;" id="intitule"><div>المقـــاول</div></td>'+
				'<td style="width : 10%;"><div> القيمة </div></td>'+
				'<td style="width : 9%;"><div> رقم الوضعية </div></td>'+
				'<td style="width : 10%;"><div> تاريخ الوضعية </div></td>'+
				'<td style="width : 5%;"><div>  رقم الحوالة </div></td>'+
				'<td style=" width : 10%;" id="intitule"><div>تـــــاريخ الدفع</div></td>';
				tds+='<td style="width : 8%; text-align : center;"><div> إختيار</div></td>';
	...
	for (var i = 0; i < engagements.length; i++) {
		...
		'<tr style="font-weight : bold '+style+'">'+
			...
			tds +='<td>'+
		    	'<span><button class="btn btn-success"  onclick="document.location.href=\'/add_att_all/'+type+'/'+op[i].p_id+'\'">اختيار</button></span>'+
		        '</td>';
