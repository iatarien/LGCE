...
<div id="main" class="row main">
	<div class="col-lg-12">
		<div class="row">
			<div class=" col-lg-offset-3 col-lg-3 form-group">
				<input  id="op_input" autocomplete="off" list="ops" dir="ltr" placeholder=" Opération" class="form-control"  onclick="op_like(this.value)" onkeyup="op_like(this.value)" > 
			...
			<div class="  col-sm-3 form-group">
				<input id="e_input" autocomplete="off" dir="ltr" placeholder="Entreprise" list="ops" class="form-control"  onclick="e_like(this.value)" onkeyup="e_like(this.value)" > 
					...
					@else
					  @foreach ($es as $e)
                      <span class="es_clss" style="cursor: pointer; text-align : left;" onclick="filter_e2('','{{ $e->id }}1989raouf1989{{ $e->name }}')">{{ $e->name  }}</span>
                      @endforeach
					@endif
                    </div>
			</div>		
	...
	<div >
		...
		<section class="panel panel-info" style="display: table;" lang="" dir="ltr">
...
function display2(engagements,value){
	...
	var tds = '<tr style="  font-weight : bolder;">'+
		        ...
		        '<td style=" width : 15%;"><div> N° d\'Opération</div></td>'+
		        '<td style=" width : 13%;" id="intitule"><div> Objet de paiement  </div></td>'+
	...
	for (var i = 0; i < engagements.length; i++) {
		...
		'<tr style="font-weight : bold">'+
			...
			if(op[i].travaux_num != null){
				tds +='<td>'+
		        '<span><h5><strong>'+op[i].travaux_type+" "+op[i].travaux_num+'</strong></h5></span>'+
		    '</td>';
			}else {
				tds +='<td>'+
		    ...
		    '</td>';
				...
				tds +='<td>'+
		    	'<span><button class="btn btn-success"  onclick="document.location.href=\'/redirect_bord/'+op[i].p_id+'\'">Selectionner</button></span>'+
		        '</td>';
...
function display(engagements,value){
	...
	var tds = '<tr style="  font-weight : bolder;">'+
		        '<td style="width : 2%;" ><div>#</div></td>'+
		        '<td style="width : 8%;"><div> N° fiche</div></td>'+
		        '<td style=" width : 20%;"><div> N° opération</div></td>'+
		        '<td style=" width : 18%;" id="intitule"><div>  Objet d\'engagement </div></td>'+
                '<td style=" width : 18%;" id="intitule"><div>Entreprise</div></td>'+
				'<td style=" width : 10%;" id="total"><div>Montant</div></td>'+
				'<td style=" width : 10%;" id="total"><div>Date Visa</div></td>'+
	...
	for (var i = 0; i < engagements.length; i++) {
		...
		'<tr style="font-weight : bold">'+
			...
			if(type == "retrait"){
				tds +='<td>'+
		    	'<span><button class="btn btn-success"  onclick="document.location.href=\'/ajouter_engagement/'+op[i].type+'/'+op[i].eng_id+'1989raouf1989'+op[i].id_op+'\'">Selectionner</button></span>'+
		        '</td>';
			}else{
				tds +='<td>'+
		    	'<span><button class="btn btn-success"  onclick="document.location.href=\'/redirect_bord/'+op[i].eng_id+'\'">Selectionner</button></span>'+
		        '</td>';
			}
