...
<div id="main" class="row main" dir="ltr">
		...
		<div class="row">
			<div class=" col-lg-3 form-group">
                ...
                <option style="visibility : hidden" value="{{$porte}}"> 
					...
					<option value="{{$portefeuille->code}}">{{$portefeuille->ministere_fr}} ({{$portefeuille->code}})</option>
					@endforeach
				</select>
			</div>
			<div class=" col-lg-3 form-group">
				<select onchange="filter('programme',this.value);" class="form-control">
					<option style="visibility : hidden">Programme</option>
					@foreach($programmes as $programme)
					<option value="{{$programme->code}}">{{$programme->designation_fr}} ({{$programme->code}})</option>
					@endforeach
				</select>
			</div>
			<div class=" col-lg-3 form-group">
				<select class="form-control" onchange="filter('source',this.value);">
					<option style="visibility : hidden"> Action</option>
	...
	<br>
		...
		<section dir="ltr" class="panel panel-info" >
			<div class="panel-heading"> Opérations Cloturés</div>
...
function display(operations,value,order){
    ...
    '<td style="cursor : pointer; width : 2%; id="0" onclick="load_ops(\'0\')" ><div>#</div></td>'+
		        '<td style="cursor : pointer; width : 14%;" id="source" onclick="load_ops(\'source\')" ><div> Numéo et intitulé </div></td>'+
				'<td style="cursor : pointer; width : 4%;" id="source" onclick="load_ops(\'source\')" ><div> Action </div></td>'+
		        '<td style="cursor : pointer; width : 10%;" id="pay_cumul" onclick="load_ops(\'pay_cumul\')" ><div>AE Actuelle  </div></td>'+
				'<td style="cursor : pointer; width : 10%;" id="eng_cumul" onclick="load_ops(\'eng_cumul\')" ><div>  cumul Engagements</div></td>'+
		        '<td style="cursor : pointer; width : 10%;" id="pay_cumul" onclick="load_ops(\'pay_cumul\')" ><div> Cumul Paiements</div></td>'+
		        '<td style="cursor : pointer; width : 9.5%; text-align : left;" id="taux" onclick="load_ops(\'taux\')" ><div>  PEC</div></td>'+
				'<td style="cursor : pointer; width : 9.5%; text-align : left;" id="taux" onclick="load_ops(\'taux\')" ><div> Solde sur AE</div></td>'+
	...
	for (var i = 0; i < operations.length - 3; i++) {
		...
		'<tr style="font-weight : bold">'+
			...
			'<strong>'+op[i].intitule+'</strong>'+
			'</span></td>'+
		    '<td>'+
		        '<span><h5><strong>'+op[i].source+'</strong></h5></span>'+
		    '</td>'+
		    '<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(op[i].AP_act)+'</strong></h5></span>'+
		    '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(op[i].somme_total_eng)+'</strong></h5></span>'+
		    '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(op[i].somme_total_pay)+'</strong></h5></span>'+
		    '</td>'+
		    '<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(op[i].AP_act-op[i].somme_total_pay)+'</strong></h5></span>'+
		    '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(op[i].AP_act-op[i].somme_total_eng)+'</strong></h5></span>'+
		    '</td>';
	...
	tds += '<tr style="font-weight : bold; background-color : #acdef6;">'+
		    ...
		    // '</td>'+
		    // '<td>'+
		    //     '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(operations[operations.length -6 ])+'</strong></h5></span>'+
		    // '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(operations[operations.length -3 ])+'</strong></h5></span>'+
		    '</td>'+
		    '<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(operations[operations.length -2 ])+'</strong></h5></span>'+
		    '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(operations[operations.length -1 ])+'</strong></h5></span>'+
		    '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(operations[operations.length -3 ] - operations[operations.length -1 ])+'</strong></h5></span>'+
		    '</td>'+
			'<td>'+
		        '<span><h5 style="text-align : left" dir="ltr"><strong>'+numberWithCommas(operations[operations.length -3 ] - operations[operations.length -2 ])+'</strong></h5></span>'+
		    '</td>'+
			'<td colspan="2" style="text-align : center">'+
